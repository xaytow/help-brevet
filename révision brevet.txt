...
# Fonction pour normaliser les chaînes (retirer les accents, passer en minuscule et garder que les lettres)
def normaliser_chaine(chaine):
    chaine = unicodedata.normalize('NFD', chaine)
    chaine = ''.join(c for c in chaine if unicodedata.category(c) != 'Mn')  # Retire les accents
    chaine = re.sub(r'[^a-zA-Z0-9]', '', chaine)  # Garde uniquement les lettres et chiffres
    return chaine.lower()
...
questions = {
    ...
    "anglais": [
        ...
        ("Translate 'Je m'appelle' into English.", "I am a student"),
...
def verifier_reponse():
    ...
    if reponse_utilisateur == normaliser_chaine(reponse_correcte.strip()):
        score += 1
        messagebox.showinfo("Résultat", f"Correct ! Bien joué. Votre score est maintenant : {score}.")
    elif reponse_correcte.strip().lower() in reponse_utilisateur:
        # Accepter si la réponse de l'utilisateur contient la réponse correcte
        score += 1
        messagebox.showinfo("Résultat", f"Votre réponse est proche. Bien joué. Votre score est maintenant : {score}.")
    else:
        messagebox.showerror("Résultat", f"Incorrect. La bonne réponse était : {reponse_correcte}. Votre score reste : {score}.")
    
    entree_reponse.delete(0, tk.END)
    nouvelle_question()

    # Vérifier si le score atteint 20 ou 50
    if score == 20:
        faire_pause()
    elif score >= 50:
        arreter_de_jouer()
...
def nouvelle_question():
    ...
    matiere = random.choice(list(questions.keys()))  # Choisir une matière aléatoire
...
def donner_indice():
    # Indice plus précis sans le premier caractère
    debut_phrase = reponse_correcte[:3]  # Prendre les 3 premiers caractères de la réponse
    indice = f"Réponse : {len(reponse_correcte)} caractères, commence par '{debut_phrase}'."
    messagebox.showinfo("Indice", indice)
...
def faire_pause():
    if messagebox.askyesno("Pause", "Vous avez atteint 20 points ! Voulez-vous faire une pause ?"):
        # Si l'utilisateur veut faire une pause, on peut mettre une logique ici
        messagebox.showinfo("Pause", "Prenez une pause bien méritée !")
        # On peut ici arrêter le flux de questions ou faire d'autres actions si nécessaire

def arreter_de_jouer():
    if messagebox.askyesno("Fin de jeu", "Vous avez atteint 50 points ! Voulez-vous arrêter de jouer ?"):
        messagebox.showinfo("Fin de jeu", f"Merci d'avoir joué ! Votre score final est : {score}.")
        app.quit()  # Quitter l'application

# Création de l'interface
app = tk.Tk()
app.title("Quiz de révision")
app.geometry("600x400")

# Label pour afficher la question
label_question = tk.Label(app, text="", wraplength=500, font=("Arial", 16))
label_question.pack(pady=20)

# Champ de saisie pour la réponse
entree_reponse = tk.Entry(app, font=("Arial", 14))  # Augmenté à 14
entree_reponse.pack(pady=10)

# Bouton pour vérifier la réponse
bouton_verifier = tk.Button(app, text="Vérifier", command=verifier_reponse, font=("Arial", 14))  # Augmenté à 14
bouton_verifier.pack(pady=10)

# Bouton pour donner un indice
bouton_indice = tk.Button(app, text="Indice", command=donner_indice, font=("Arial", 14))  # Augmenté à 14
bouton_indice.pack(pady=10)

# Affichage du score
label_score = tk.Label(app, text=f"Score : {score}", font=("Arial", 16))  # Augmenté à 16
label_score.pack(pady=10)

# Boutons pour insérer des caractères spéciaux en langues
frame_langues = tk.Frame(app)
frame_langues.pack(pady=10)
caracteres_speciaux_langues = ['é', 'à', 'â', 'ê', 'ô', 'î', 'ç', 'œ', '€', 'ñ', '¿', '¡']
for caractere in caracteres_speciaux_langues:
    bouton_caractere = tk.Button(frame_langues, text=caractere, command=lambda c=caractere: ajouter_caractere(c), font=("Arial", 14))  # Augmenté à 14
    bouton_caractere.pack(side=tk.LEFT, padx=2)

# Boutons pour insérer des caractères spéciaux en mathématiques
frame_math = tk.Frame(app)
frame_math.pack(pady=10)
caracteres_speciaux_math = ['π', '∞', '√', '∫', '∑', '≈', '≤', '≥', '≠', '÷', '×']
for caractere in caracteres_speciaux_math:
    bouton_caractere = tk.Button(frame_math, text=caractere, command=lambda c=caractere: ajouter_caractere(c), font=("Arial", 14))  # Augmenté à 14
    bouton_caractere.pack(side=tk.LEFT, padx=2)
